...
//	///////////////////////// static services /////////////////////////////////////	
	...
	public static ProcessInstance create(ProcessDefinition def, String name, Map options) throws Exception{
		...
		if(USE_CLASS==null){
			try{
				USE_CLASS = Class.forName(GlobalContext.getPropertyString("processinstance.class"));
			}catch(Exception e){
...
/*		if(GlobalContext.logLevelIsDebug && !(activity instanceof ProcessDefinition))
		...
		try {
			if (GlobalContext.logLevelIsDebug) {
				PrintStream ps = null;
				try {
					File f = new File(FormActivity.FILE_SYSTEM_DIR + "/log/" + getInstanceId() + "/");
					f.mkdirs();
					
					ps = new PrintStream(
							new FileOutputStream(FormActivity.FILE_SYSTEM_DIR + "/log/" /*+ UEngineUtil.getCalendarDir() + "/" */+ getInstanceId() + "/" + activity.getTracingTag() + ".log.xml")
					);
