            ...
            if (nodes[i].nodeType == 1) {
                ...
                if (nodes[i].firstChild) {
                    if (nodes[i].firstChild.tagName == "A") {
                        ...
                        if (a.firstChild) {
                            var inputCheckbox = a.firstChild;
                            node.type  = inputCheckbox.type;
                            node.id    = inputCheckbox.id;
                            node.value = inputCheckbox.value;
                        }
                        if (a.lastChild) {
                            node.text = a.lastChild.nodeValue.trim();
                        }
                        if (a.href) {
                            node.href = a.href;
                        }
    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
        ...
        this.toHtml = function() {
            ...
            if (this.isDoc) {
                ...
                s += '<a href="?"??><input type=? id=? value=?>?</a>'.format(this.href, (this.title ? ' title="?"'.format(this.title) : ""), (this.target ? ' target="?"'.format(this.target) : ""), this.type, this.id, this.value, this.text);
            }
            s += '</div>';
            return s;
        };
