...
<head>
	...
	<script type="text/javascript" src="../../scripts/formApprovalScript.js"></script>
...
//			boolean wasIsJBoss = "JBOSS".equals(GlobalContext.getPropertyString("was.type", "TOMCAT"));
		...
		%>
					...
					<% if (!isDraftActivity && !isCompleted) { %>
						...
						<div id="divAppComment" style="display: none"></div>
						<textarea id="approvalComment" name="approvalComment" ></textarea><br />
						<script type="text/javascript">
						CKEDITOR.replace('approvalComment',
						{
		...
		<%
							...
							arrPdr = pm.findAllVersions(procDef.getBelongingDefinitionId());
							if(arrPdr != null){
		...
		%>
							if ('null' == '<%=pdVer%>') {
								...
								this.location='../../processmanager/initiateForm.jsp?processDefinition=<%=pdVer%>&isCopyInitiate=yes&fckEditorContents=&copyProcInstId='+procInstId+'&copyTracingTag='+tracingTag;
							}
						}
				</script>
		<%
