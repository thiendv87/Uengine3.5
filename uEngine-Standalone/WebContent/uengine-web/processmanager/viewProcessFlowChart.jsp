...
<link rel="stylesheet" type="text/css" href="<%=GlobalContext.WEB_CONTEXT_ROOT %>/style/portlet.css" />
<link rel="stylesheet" type="text/css" href="<%=GlobalContext.WEB_CONTEXT_ROOT %>/style/portal.css" />
<link rel="stylesheet" type="text/css" href="<%=GlobalContext.WEB_CONTEXT_ROOT %>/style/groupware.css" />
...
var tmp = [
           ...
           {title : "<%=GlobalContext.getMessageForWeb("Process Variables", loggedUserLocale) %>", onclick : "changeDisplayDraw(false)"},
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td valign="middle"><img src="../images/Common/I_info.gif" align="middle" border="0" style="margin-top:-2px;"> <strong ><%=GlobalContext.getMessageForWeb("Process Information", loggedUserLocale) %></strong></td>
        <td>
<% 
if (pdVer != null) { 
%>
...
%>
                                        	<a href="javascript: viewTaskInfo('<%=pdVer%>')"> <span> <%=GlobalContext.getMessageForWeb("Initiate", loggedUserLocale) %> </span> </a>
<% 
    }
...
%>
	                                        ...
	                                        <span><%=GlobalContext.getMessageForWeb("Improve", loggedUserLocale) %></span></a>
<%
    }
                                        
    if (pdr.isProduction()) {
%>
                                            <span style="line-height:23px; color:#F00">[<%=GlobalContext.getMessageForWeb("This version is procuction", loggedUserLocale) %>]</span>
<%
    } else if (loggedUserIsAdmin) {
%>
                                            <a href="javascript: makeProduction();"> <span> <%=GlobalContext.getMessageForWeb("Set as production", loggedUserLocale)%> </span> </a>
<%
    }
%>
										</td>
...
%>
                        <a href="javascript: window.location.href='processInstanceListDetail.jsp?defId=<%=pdr.getBelongingDefinitionId() %>'"> <span> <%=GlobalContext.getMessageForWeb("Instance List", loggedUserLocale) %> </span> </a>
<% 
} 
%>
                	</td>
...
%>
    <tr height="30" >
        <td class="formtitle" width="200">&nbsp;<b><%=GlobalContext.getMessageForWeb("Definition", loggedUserLocale) %></b></td>
        <td width="10"></td>
        <td width="*" align="left"><%=pdr.getName().getText()%> ( <%=GlobalContext.getMessageForWeb("ID", loggedUserLocale) %> : <%=pdr.getBelongingDefinitionId()%> , <%=GlobalContext.getMessageForWeb("Alias", loggedUserLocale) %> : <%=srAlias %> ) </td>
    </tr>
    <tr>
        <td colspan="3" class="formline" height="1"></td>
    </tr>
...
%>
    <tr height="30" >
        <td class="formtitle">&nbsp;<strong><%=GlobalContext.getMessageForWeb("Version", loggedUserLocale) %></strong></td>
        <td width="10"></td>
        <td width="*" align="left">
...
%>
            ...
            ( <%=GlobalContext.getMessageForWeb("ID", loggedUserLocale) %> : <%=pdVer%> , <%=GlobalContext.getMessageForWeb("Modified Date", loggedUserLocale) %> : <%=pdr.getStrModifiedDate() %> ) 
        </td>
    </tr>
    <tr>
        <td colspan="3" class="formline" height="1"></td>
...
%>
    <tr height="30" >
        <td class="formtitle">&nbsp;<b><%=GlobalContext.getMessageForWeb("Description", loggedUserLocale) %></b></td>
        <td width="10"></td>
...
if ( pdVer == null) {
	...
	if(pdr.isAdhoc()) { 
%>
                                        <a onClick="location='ProcessDesigner.jnlp?instanceId=<%=pi%>&defId=<%=pdr.getId()%>'"><span><%=GlobalContext.getMessageForWeb("Instance Level Definition Change", loggedUserLocale) %></span></a>
...
%>
							...
							<select onchange="changeLocale(this)">
								<option value=""><%=GlobalContext.getMessageForWeb("Language", loggedUserLocale) %></option>
								<option value="ko"><%=GlobalContext.getMessageForWeb("Korean", loggedUserLocale) %></option>
								<option value="en"><%=GlobalContext.getMessageForWeb("English", loggedUserLocale) %></option>
								<option value="jp"><%=GlobalContext.getMessageForWeb("Japanese", loggedUserLocale) %></option>
								<option value="zh"><%=GlobalContext.getMessageForWeb("Chinese", loggedUserLocale) %></option>
							</select>
							<script type="text/javascript">
...
%>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr height="30" >
            <td class="formtitle" width="200">&nbsp;<b><%=GlobalContext.getMessageForWeb("See definition in", loggedUserLocale) %></b></td>
            <td width="10"></td>
            <td width="*" align="left"  class="padding15">
...
<div id="divTabItem_<%=GlobalContext.getMessageForWeb("Process Variables", loggedUserLocale) %>" style="display: none;">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr height="30" >
...
%>
                            <input type="button" value='<%=GlobalContext.getMessageForWeb("Change", loggedUserLocale) %>' 
