function makeSchedulerCronExpression(elementId) {
	var displayElement = document.getElementById(elementId);
	var realElement =  document.getElementsByName(elementId.split("_")[1])[0];
	
	var url = WEB_ROOT + "/processmanager/schedulerCronExpressionAction.jsp";
	var StrOption;
	StrOption = "dialogWidth:500px;dialogHeight:230px;";
	...
	if( arrResult != null && arrResult[0] != null ){
		realElement.value = arrResult[0];
		displayElement.value = arrResult[1];
	}
}
...
function searchPeopleObj(obj, multiple){
	...
	var orgPicker = window.open(WEB_ROOT + '/common/organizationChartPicker.jsp?multiple=' + multiple + '&elemid=' + elemid, '_new','width=400,height=550,resizable=true,scrollbars=no');
...
function fcnOpenActivity(pwidth,pheight,paddr,pname,pscroll)
{
