function fcnOpen(pwidth,pheight,paddr,pname,pscroll)
{
...
function fcnPostOpen(pformname,pwidth,pheight,paddr,pname,pscroll)
{
	var lf_tempx = (screen.availWidth - pwidth) / 2;
	var lf_tempy = (screen.availHeight - pheight) / 2 - 20;
	
	var tempopt = "width="+pwidth+",height="+pheight+",toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars="+pscroll+",resizable=0,left="+lf_tempx+",top="+lf_tempy;
	
	var mpname = window.open("",pname,tempopt);
	//var add = document.all.pformname.REQ_CD.value;
	//var paddr = paddr+"?posid="+add;
	//alert(paddr);
	eval("document."+pformname+".target='"+pname+"'");
	eval("document."+pformname+".action='"+paddr+"'");
...
function fcnCommonPreview(type)
{
	var f = document.getElementById("factory");
	
	f.printing.header			= '&w&bPage &p / &P';
	f.printing.footer			= 'Service Request Management System &b&d';
	f.printing.portrait			= type
	f.printing.leftMargin		= 10.0
	f.printing.topMargin		= 10.0
	f.printing.rightMargin		= 10.0
	f.printing.bottomMargin		= 10.0
	f.printing.templateURL 		= "MeadCo://IE7"
	f.printing.Preview();

}

function fcnStyle(obj)
{
	obj.onfocus = function()
	{
		obj.style.background="#ffe3ee";
	}
	obj.onblur = function()
	{
...
function popupSearchSR(){
	var url = "/srms/common/component/serviceSearch?seqval=" + document.getElementById('seqval').value;
	window.open(url);
}

function fcnOnChange()
{
    document.forms[0].spjt_tp2.options[0].text = 'Loading....';
    //var parameter = '&allowValue=all&grp_cd='+document.forms[0].spjt_tp1.value;
    var parameter = '&grp_cd='+document.forms[0].spjt_tp1.value;
    
	document.getElementById(document.forms[0].spjt_tp1.name + "_target").value = document.forms[0].spjt_tp1.value;
    sendRequest(fcnSetCodeValue, parameter,'post','/srms/common/code/comboBoxList',true,true);
}

function fcnSetCodeValue(oj)
{
	...
	document.forms[0].spjt_tp2.length = rows.length-1;
    ...
    for(i=0;i<rows.length-1;i++)
    {
        ...
        {
            cols = rows[i].split(":");
            document.forms[0].spjt_tp2.options[i].value = cols[0];
            document.forms[0].spjt_tp2.options[i].text = cols[1];
        }

    }
}
//창의 크기를 넘겨 받지 않았을 경우는 아래의 함수를 사용합니다.
function fileuploadPopup(divnm){
	var strDivNms = divnm.split("[");
	var strDivIndex = "";
	if (strDivNms[1]) {
		strDivIndex = "[" + strDivNms[1];
	}
	var seqField = document.getElementById(strDivNms[0] + "_seq" + strDivIndex);
	var url = "/fileupload/fileUploadForm.jsp?divnm=" + divnm + "&seq=" + seqField.value;
	str = "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=480,height=280";
	var pwidth=490;//파일첨부 창 가로 크기
	var pheight=314;//파일첨부 창 세로 크기
	var lf_tempx = (screen.availWidth - pwidth) / 2;
	var lf_tempy = (screen.availHeight - pheight) / 2 - 20;
	var lf_tempopt = "width="+pwidth+",height="+pheight+",toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=yes,resizable=1,left="+lf_tempx+",top="+lf_tempy;
	var win=window.open(url, "win",lf_tempopt);

}
//창의 크기를 넘겨 받을 경우는 아래의 함수를 사용합니다.
function fileuploadPopupSub(divnm,pwidth,pheight){
	var strDivNms = divnm.split("[");
	var strDivIndex = "";
	if (strDivNms[1]) {
		strDivIndex = "[" + strDivNms[1];
...
function fcnFileSampleDown(fileCode,groupCode){
		
		var url =  "/fileupload/fileDownload.jsp?sample=Y&downfilename=" + fileCode+"&groupCode="+groupCode;
		var str = "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=100,height=50";

		self.location.href=url;
}

function fileDown(fileCode, fileName){
	var url = "/fileupload/fileDownload.jsp?seq=" + fileCode + "&downfilename=" + fileName;
	var str = "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=480,height=280";
	//window.open(url, "Filedown", str);
	self.location.href=url;
}
