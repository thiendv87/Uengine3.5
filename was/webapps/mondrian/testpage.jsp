<%@ page session="true" contentType="text/html; charset=ISO-8859-1" import="java.util.*, java.io.*, org.uengine.kernel.ProcessDefinitionFactory" %>
...
<fa:form id="chartform02" xmlUri="/WEB-INF/jpivot/favor/chart/chartpropertiesform.xml" model="#{chart02}" visible="false"/>

<wcf:table id="query01.drillthroughtable" visible="false" selmode="none" editable="true"/>

<fa:favorite id="favor01" chartmodel="chart02" model="query01"/>
	
<fa:dynamicform id="favorform" xmlURL="/favorite.jsp" model="#{favor01}" visible="false"/>
<fa:dynamicform id="favorsave" xmlURL="/favoritesave.jsp" model="#{favor01}" visible="false"/>
...
<wcf:toolbar id="toolbar01" bundle="com.tonbeller.jpivot.toolbar.resources">
  ...
  <!--wcf:separator/-->
  <wcf:scriptbutton id="chartButton02" tooltip="toolb.chart" img="chart" model="#{chart02.visible}"/>
  <wcf:scriptbutton id="chartPropertiesButton02" tooltip="toolb.chart.config" img="chart-config" model="#{chartform02.visible}"/>
  <wcf:separator/>
  ...
  <wcf:separator/>
  <wcf:scriptbutton id="favorite" tooltip="toolb.dashboard.open" img="dashboard-open" radioGroup="dashboard" model="#{favorform.visible}"/>
  <wcf:scriptbutton id="favoritesave" tooltip="toolb.dashboard.save" img="dashboard-save" radioGroup="dashboard" model="#{favorsave.visible}"/>
</wcf:toolbar>
...
<%-- render open --%>
<c:if test="${favorform.visible}">
	
				Open Existing Favorite View: <select name="changeFavoriteName">
				<%
						...
						for(int i=0; i<files.length; i++){		
							if(files[i].getName()!=null && files[i].getName().startsWith("favorite_")){
								%><option value="<%=files[i].getName()%>"> <%=files[i].getName().substring(9)%></option><%
							}
						}
				%>
				</select>			
</c:if>
<wcf:render ref="favorform" xslUri="/WEB-INF/wcf/wcf.xsl" xslCache="true"/>
<wcf:render ref="favorsave" xslUri="/WEB-INF/wcf/wcf.xsl" xslCache="true"/>
...
<%-- FusionCart properties --%>
<wcf:render ref="chartform02" xslUri="/WEB-INF/wcf/wcf.xsl" xslCache="true"/>
