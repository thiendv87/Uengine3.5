...
<%
	ResultPayload resultPayload = new org.uengine.kernel.ResultPayload();
		...
		if (isEventHandler) {
		...
		} else if (initiate || saveOnly ||"draft".equals(apprStat)) {//The case that this workitem handler should initiate the process
			if (initiate) {
			...
			FormApprovalLineActivity approvalLineActivity = (FormApprovalLineActivity) processDefinitionObject.getActivity(approvalLineActivityTT);
			...
			if (!isSwitchActivityExist) {
				...
				String preType = null;
				FormApprovalActivity draftActivity = approvalLineActivity.getDraftActivity();
				...
				for (int i = 1; i < approvalInfoCodes.length; i++) {
					...
					FormApprovalActivity approvalActivity = (FormApprovalActivity) approvalLineActivity.getDraftActivity().clone();
					...
					approvalActivity.setName(userName + " (" + (("1".equals(approveType))?"결재":"동의") + ")");
					...
					if (!"21".equals(preType) && "21".equals(approveType)) {
						AllActivity allAct = new AllActivity();
						parent.addChildActivity(allAct);
						parent = allAct;
					} else if ("21".equals(preType) && !"21".equals(approveType)) {
			...
			FormApprovalActivity currentRunningActivity = (FormApprovalActivity) instance.getCurrentRunningActivity().getActivity();
			...
			if ("draft".equals(apprStat)) {
				...
				approvalLineActivity = (FormApprovalLineActivity) processDefinitionObject.getActivity(approvalLineActivityTT);

				boolean loadApprovealActivities = "yes".equals(getParameter(parameterMap, "loadApprovealActivities"));

				WorkListDAO wl = (WorkListDAO) ConnectiveDAO.createDAOImpl(instance.getProcessTransactionContext(),
				...
				if (!isSwitchActivityExist && loadApprovealActivities) {
					...
					for (int i = 1; i < approvalInfoCodes.length; i++) {
						...
						FormApprovalActivity approvalActivity = (FormApprovalActivity) approvalLineActivity.getDraftActivity().clone();
						//approvalActivity.setName(approvalLineActivity.getDraftActivity().getName().getText() + " " + userName + " (" + (("1".equals(approveType))?"결재":"동의") + ")");
						approvalActivity.setName(userName + " (" + (("1".equals(approveType))?"결재":"동의") + ")");
						approvalActivity.setTracingTag(null); //let it auto-generated
						approvalActivity.setRole(null);
						approvalActivity.setViewMode(true);
					...
					FormApprovalLineActivity alaTemp = (FormApprovalLineActivity) pdTemp.getActivity(approvalLineActivityTT);
					...
					if (child.size() > 1) {
						...
						for (int i = 0; i < removeTargetActivity.size(); i++) {
							FormApprovalActivity act = (FormApprovalActivity) removeTargetActivity.get(i);
							FormApprovalActivity clonedAct = (FormApprovalActivity) act.clone();
				...
				if (changedApprovalLine) {
					...
					approvalLineActivityTT = getParameter(parameterMap, "approvalLineActivityTT");
					approvalLineActivity = (FormApprovalLineActivity) clonedProcessDefinitionObject.getActivity(approvalLineActivityTT);
					//processDefinitionObject = pm.getProcessDefinition(initiationProcessDefinition);
					String approvalLineUserCodes = getParameter(parameterMap, "userCode");
					String approvalLineUserNames = getParameter(parameterMap, "userName");
					//String approvalLineUserJobPosition = getParameter(parameterMap, "userJobPosition");
					...
					int j = 0;
					for (int i = 0; i < appActList.size(); i++) {
						if ( appActList.get(i) instanceof FormApprovalActivity ){
							FormApprovalActivity act = (FormApprovalActivity) appActList.get(i);
					...
					for (int addIdx = 0; addIdx < addActSize; addIdx++) {
						...
						//		+ approvalInfoNames[arrayIndex] + " (" + (("1".equals(approvalInfoTypes[arrayIndex]))?"결재":"동의") + ")");
						approvalActivity.setName(approvalInfoNames[arrayIndex] + " (" + (("1".equals(approvalInfoTypes[arrayIndex]))?"결재":"동의") + ")");
						approvalActivity.setTracingTag(null); //let it auto-generated
						approvalActivity.setRole(null);
						approvalActivity.setViewMode(true);
				...
				((FormApprovalActivity) processDefinitionObject.getActivity(tracingTag)).setApprovalStatus(pm.getProcessInstance(processInstance),
						ApprovalActivity.SIGN_APPROVED);
				pm.completeWorkitem(processInstance, tracingTag, taskId, resultPayload);

				String comment = URLDecoder.decode(request.getParameter("approvalComment"), "UTF-8");
				...
				((FormApprovalActivity) processDefinitionObject.getActivity(tracingTag)).arbitraryApprove(pm.getProcessInstance(processInstance));
				...
				((FormApprovalActivity) processDefinitionObject.getActivity(tracingTag)).rejectApprove(pm.getProcessInstance(processInstance));
				String comment = request.getParameter("approvalComment");
...
%>
				var url = CONTEXT_ROOT + "/wih/formApprovalHandler/submitResult.jsp";
