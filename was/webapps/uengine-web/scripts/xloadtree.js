...
function setAt(key, value){
  ...
  for (var i = 0; i < this.treeMap.length; i++)
  {
    if ( this.treeMap[i].key === key )
    {
      this.treeMap[i].value = value;
      return;
    }
  }
  
  this.treeMap[this.treeMap.length] = new struct(key, value);
...
function isEmpty(){
  return this.treeMap.length <= 0;
}

function classTreeMap() {

  this.treeMap = new Array();
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon, contextAction) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, contextAction);
...
function _xmlTreeToJsTree(oNode) {
	...
	var contextAction = oNode.getAttribute("contextAction");
	
	//alert(contextAction);
	
	//if ( oNode.text != '') text = oNode.text;
	
	var objNodeList = oNode.getElementsByTagName("text");
	if ( objNodeList.length > 0 ) {
		text = objNodeList.item(0).text;
		//alert(objNodeList.item(0).text);
	}

	var id;
	var objIdNodeList = oNode.getElementsByTagName("id");
	if ( objIdNodeList.length > 0 ) {
		id = objIdNodeList.item(0).text;
		//alert(objNodeList.item(0).text);
	}
	
	treeMap.setAt("'" + id + "'", text);
	
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon, contextAction);
	else
		jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon, contextAction);
