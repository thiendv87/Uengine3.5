...
| 2004-04-07 | Added a try/catch around getDocument because there are some    |
|            | cases where it might actually fail.                            |
| 2004-08-02 | Added support for rightToLeft to Menu                          |
|-----------------------------------------------------------------------------|
| Dependencies: poslib.js       Used to find positions of elements            |
|               scrollbutton.js	Used for the buttnos that allows the menu     |
|                               to be scrollable                              |
|-----------------------------------------------------------------------------|
| Created 2002-05-28 | All changes are in the log above. | Updated 2004-08-02 |
\----------------------------------------------------------------------------*/
...
Menu.prototype.invalidate = function () {
	if ( this._drawn ) {
		...
		for ( var i = 0; i < l; i++ ) {
			...
			if ( mi._htmlElement )
			{
				mi._htmlElement._menuItem = null;
...
{
	if ( this._htmlElement )
		return this._htmlElement.currentStyle.direction == "rtl";
	return false;	// fall back
};
