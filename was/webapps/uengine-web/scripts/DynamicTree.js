...
function DynamicTree(id, rootpath) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].nodeType == 1) {
                ...
                if (nodes[i].firstChild) {
                	...
                	var theChildNode = nodes[i].firstChild;
                	
                	if (nodes[i].firstChild.tagName == "IMG") {
                        var img = nodes[i].firstChild;
                       	node.iconsrc = img.src;
                       	node.text = img.alt;
                       	
                       	theChildNode = nodes[i].childNodes[1];
	                }
	                      
	                if(theChildNode){ 	
	                    if (theChildNode.tagName == "A") {
	                        var a = theChildNode;
	                        if (a.firstChild) {
	                        ...
	                        node.text = theChildNode.nodeValue.trim();
	                    }
	                }
                }
    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
        ...
        this.toHtml = function() {
            ...
            if (this.isFolder) {
                ...
                //alert(this.imgsrc);
      			if(this.iconsrc != "") icon = this.iconsrc;
