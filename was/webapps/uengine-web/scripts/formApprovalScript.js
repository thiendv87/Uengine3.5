	...
	function viewCommentJust(htmlData){
		var jDiv = jQuery(document.createElement("div"));
		jDiv.attr("title", "Comment for approval");
		jDiv.html(htmlData);
		jDiv.dialog({
	...
	function addApprovalLine(isChangeApprovalLine) {
		var empno = document.all.empno.value;
		var activityStatus = document.all.activityStatus.value;
		var appvtype = document.all.appvtype.value;
		var initype = document.all.initype.value;
		
		var orgPicker = fcnOpenApproval(900,650, WEB_CONTEXT_ROOT + '/wih/formApprovalHandler/approvalLineEditor/flow_edit_frame.jsp?isChangeApprovalLine=' + isChangeApprovalLine + "&empno="+empno + "&activityStatus="+activityStatus + "&appvtype=" + appvtype + "&initype=" + initype, '_new', false);
	...
	function confirmWorkitem(instanceId,tracingTag,approveType){
		...
		var compComment = $("#divAppComment").html(CKEDITOR.instances.approvalComment.getData()).text();

		var commentLength = compComment.length;//comment.length;
		...
		$("#divAppComment").html(CKEDITOR.instances.approvalComment.getData());
		var comment = $("#divAppComment").html();
