...
Array.prototype.addPoint = function(sPoint, ePoint) {
	var point = [sPoint, ePoint];
	this[this.length] = point;
}

Array.prototype.addBezierPoint = function(sPoint, cPoint, ePoint) {
...
function drawLine(canvasId, points, color, lineWidth) {
	//Create jsGraphics object
	var gr = new jsGraphics(document.getElementById(canvasId));
    ...
    var pen = new jsPen(col, lineWidth);

    //Draw a Line between 2 points
    for (var i = 0; i < points.length; i++) {
    	var point = points[i];
        if (point.isBezier) {
        	gr.drawPolyBezier(pen, point);
        } else {
...
function drawLoopLine(stroke, namespace, tracingTag) {
	...
	if (isVertical) {
		...
		var startPoint = new jsPoint(startX, startY);
		var curve01SPoint = new jsPoint(loopTopRightX, startY);
		var curve01CPoint = new jsPoint(loopTopRightX + lineWidth, startY);
		var curve01EPoint = new jsPoint(loopTopRightX + lineWidth, startY + lineWidth);
		var curve02SPoint = new jsPoint(loopTopRightX + lineWidth, switchY - lineWidth);
		var curve02CPoint = new jsPoint(loopTopRightX + lineWidth, switchY);
		var curve02EPoint = new jsPoint(loopTopRightX - lineWidth, switchY);
		var lastPoint = new jsPoint(switchX, switchY);
		...
		points.addPoint(curve02EPoint, lastPoint);

		startX += 4;
		var arrowPoint = new jsPoint(startX, startY);
		points.addPoint(arrowPoint, new jsPoint(startX + (lineWidth / 2), startY - lineWidth / 2));
		points.addPoint(arrowPoint, new jsPoint(startX + (lineWidth / 2), startY + lineWidth / 2));
		...
		var startPoint = new jsPoint(startX, startY);
		var curve01SPoint = new jsPoint(startX, loopY);
		var curve01CPoint = new jsPoint(startX, loopY - lineWidth);
		var curve01EPoint = new jsPoint(startX + lineWidth, loopY - lineWidth);
		var curve02SPoint = new jsPoint(switchX - lineWidth, loopY - lineWidth);
		var curve02CPoint = new jsPoint(switchX, loopY - lineWidth);
		var curve02EPoint = new jsPoint(switchX, loopY);
		var lastPoint = new jsPoint(switchX, startY);
	...
	drawLine('canvasForLoopLines' + namespace, points, "#888888", 2);
