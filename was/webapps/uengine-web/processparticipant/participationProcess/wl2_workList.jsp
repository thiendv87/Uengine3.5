...
<%
	...
	*/
	...
	if (UEngineUtil.isNotEmpty(simpleKeyWord)) {
		...
		strAddWhere.append("	").append(lowerCaseFunctionName).append("(wl.TITLE) 		LIKE '%").append(simpleKeyWordLowerCase).append("%' ").append(" OR ");
		...
		if (UEngineUtil.isNotEmpty(strWorkName)) {
			strAddWhere.append(" AND wl.title LIKE '%" + strWorkName + "%' "); 
		}

		if (UEngineUtil.isNotEmpty(strInstanceName)) {
			strAddWhere.append(" AND inst.name LIKE '%" + strInstanceName + "%' "); 
		}

		if (UEngineUtil.isNotEmpty(strWorkitemStartDate)) {
			strAddWhere.append(" AND inst.starteddate >= '" + strWorkitemStartDate + "' "); 
		}

		if (UEngineUtil.isNotEmpty(strWorkitemEndDate)) {
			strAddWhere.append(" AND inst.starteddate < '" + strWorkitemEndDate + "' "); 
		}
		
		if (UEngineUtil.isNotEmpty(search_user_display)) {
			strAddWhere.append(" AND inst.initrsnm = '" + search_user_display + "' "); 
		}
		
		if (UEngineUtil.isNotEmpty(search_user)) {
			strAddWhere.append(" AND inst.initep = '" + search_user + "' "); 
		}

		if(UEngineUtil.isNotEmpty(processDefName)){
			strAddWhere.append(" AND inst.defname LIKE '%"+ processDefName +"%'");
		}
	}
	...
	String[] filters = {
		...
		"(wl.status = 'NEW' or wl.status = 'CONFIRMED' or wl.status = 'DRAFT')",
	...
	*/
	...
	if (filteringNo == 4) {
		...
		if ("ORACLE".equals(typeOfDBMS))
			...
			filters[filters.length-1] += " and (TO_CHAR(wl.duedate,'yyyy-MM-dd HH24:mm:ss') < TO_CHAR(CURRENT_TIMESTAMP,'yyyy-MM-dd HH24:mm:ss'))";
		else if ("MYSQL".equals(typeOfDBMS))
			filters[filters.length-1] += " and wl.duedate < now()";
	}
...
%>
<title><%=GlobalContext.getMessageForWeb("Work List", loggedUserLocale) %> - <%=filterName %></title>
</head>

<body  alink="black" vlink="black" onLoad="setListForm(document.refreshForm);">

<div id="divSubSearch" style="display: none;" title="<%=GlobalContext.getMessageForWeb("Advenced Search", loggedUserLocale) %>">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
        ...
        <tr>
            ...
            <td class="formcon"><input type="text" id="inputInstanceName" value="<%=strInstanceName %>" size='15' /></td>
            <td class="formtitle"><%=GlobalContext.getMessageForWeb("Work Name", loggedUserLocale) %></td>
            <td class="formcon"><input type="text" id="inputWorkName" value="<%=strWorkName %>" size='15' /></td>
        </tr>
        <tr bgcolor="#b9cae3">
        ...
        <tr>
            ...
            <td class="formcon" >
                <input type='text' id="workitem_start_date" value="<%=strWorkitemStartDate %>" size="10" />
                <img align="middle" src='../../processmanager/images/calendar.gif' name="workitem_start_date" onclick="showFullcalendar('month', this.name);" /> ~
                <input type='text' id="workitem_end_date" value="<%=strWorkitemEndDate %>" size="10" />
                <img align="middle" src='../../processmanager/images/calendar.gif' name="workitem_end_date" onclick="showFullcalendar('month', this.name);" />
			</td>
            <td class="formtitle" ><%=GlobalContext.getMessageForWeb("ProcessDefinition", loggedUserLocale) %></td>
            <td class="formcon" ><input type='text' id='processDefName' size='15' value="<%=processDefName %>" onKeyDown="javascript:resetHiddenValue('ProcessDefinition');"/>
        ...
        <tr>
            ...
            <td class="formcon" >
                <input type="text" id='search_user' size='15' value="<%=search_user %>" />
                <img name="search_user" align="middle" onclick='searchPeopleObj(this, false)' style="cursor: pointer;" 
					src='<%=GlobalContext.WEB_CONTEXT_ROOT%>/processmanager/images/Toolbar-toblock.gif' />
			</td>
               <td class="formtitle" ><%=GlobalContext.getMessageForWeb("Initiator Name", loggedUserLocale) %></td>
            <td class="formcon" ><input type="text" id='search_user_display' size='15' value="<%=search_user_display %>" /><input type="hidden" id='search_user__which_is_xml_value' value="<%=search_user__which_is_xml_value %>" /></td>
        </tr>
    </table>
</div>
    ...
    <table border='0' width='98%' align="center" cellpadding='0' cellspacing='0'>
        <tr>
            <td valign='top'>
                ...
                <legend><%=GlobalContext.getMessageForWeb("WorkItem Search", loggedUserLocale) %></legend>
                        	...
                        	<table width="*" border="0" cellspacing="0" cellpadding="0">
                                <tr height="25" valign="middle">
                                    ...
                                    <td background="<%=GlobalContext.WEB_CONTEXT_ROOT%>/images/Common/searchTitleCenter.gif">
                                        <a href="javascript: showDetailSearch();" style="text-decoration:underline;"><%=GlobalContext.getMessageForWeb("Advenced Search", loggedUserLocale) %></a></td>
                                    <td>
...
<%
	...
	*/
	...
	defListFields[1] = new ListField(GlobalContext.getMessageForWeb("Process Name", loggedUserLocale) , new InstanceTableListFieldType("procinstnm"));
	//defListFields[1] = new ListField(GlobalContext.getMessageForWeb("Process Instance", loggedUserLocale) , new InstanceTableListFieldType("NAME"));
...
*/	
	listFields = defListFields;
	sql = " select DISTINCT INST.NAME as procinstnm, INST.initrsnm, INST.INFO, WL.* FROM BPM_PROCINST INST, ";
	sql += "BPM_WORKLIST WL INNER JOIN BPM_ROLEMAPPING ROLE ON (WL.ROLENAME=ROLE.ROLENAME OR WL.REFROLENAME=ROLE.ROLENAME) ";
	if ("self".equals(endpointType)) {
		sql += "OR WL.ENDPOINT='" + loggedUserId + "' ";
	}
	sql += " where " + filter;
	sql += " AND WL.INSTID=INST.INSTID AND WL.INSTID=ROLE.INSTID AND INST.ISDELETED=0 AND INST.STATUS<>'Stopped' AND INST.STATUS<>'Failed' ";
	if ("self".equals(endpointType)) {
